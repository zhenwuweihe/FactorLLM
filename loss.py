...
def DistillLoss(input: torch.Tensor, target: torch.Tensor):
    # assert input.dtype == torch.float,  f'invalid input dtype: {input.dtype}, expect torch.float'
    assert input.shape == target.shape, f'shape of input and target must be same'
    # assert torch.allclose(torch.sum(input, axis=-1), 1) and torch.allclose(torch.sum(target, axis=-1), 1), \
    #     f"invalid input or target: expect distribution as input"

    # 防止对数计算时出现负数或零值，可以设置一个小的正数 ε（如 1e-7）作为阈值
    epsilon = 1e-7

    # 避免出现 log(0)，将 p 和 q 中小于 ε 的值替换为 ε，大于 1 - ε 的值替换为 1 - ε
    input = torch.clip(input, epsilon, 1 - epsilon)
    target = torch.clip(target, epsilon, 1 - epsilon)

    # 计算KL散度
    kl = torch.sum(input * torch.log(input / target), axis=-1)

    # 如果输入是二维数组（`(num_samples, num_classes)`），返回平均KL散度；否则返回单个KL散度值
    return torch.mean(kl) if len(input.shape) == 2 else kl[0]